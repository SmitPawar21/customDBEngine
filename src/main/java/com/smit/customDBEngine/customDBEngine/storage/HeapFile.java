...
public class HeapFile {
	...
	public void insert (Record record) throws IOException {
		...
		boolean success = p.insertRecord(data);
	...
	public boolean update(int pageIndex, int slotId, Record record) throws IOException {
		byte[] newData = record.serialize();
	...
	public Iterable<Record> scan(Schema schema) throws IOException {
		List<Record> results = new ArrayList<>();
		
		for(int pageId : pages) {
			SlottedPage p = (SlottedPage) bp.getPage(pageId);
			for(byte[] recBytes : p.getAllRecords()) {
				results.add(Record.deserialize(schema, recBytes));
			}
		}
